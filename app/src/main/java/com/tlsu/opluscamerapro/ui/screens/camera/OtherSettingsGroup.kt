...
fun OtherSettingsGroup(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.other_settings_title),
            style = MaterialTheme.typography.titleLarge,
            modifier = Modifier.padding(bottom = 16.dp, start = 8.dp)
        )
        
        // 示例设置卡片（当有实际设置时可以添加多个卡片）
        SettingsCard(title = stringResource(R.string.other_settings_category_other)) {
            // 这里可以添加其他设置项
            Text(
                text = stringResource(R.string.other_settings_no_settings),
